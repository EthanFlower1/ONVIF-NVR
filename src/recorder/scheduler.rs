...
impl RecordingScheduler {
    ...
    pub fn new(
        db_pool: Arc<PgPool>,
        stream_manager: Arc<crate::stream_manager::StreamManager>,
    ...
    async fn process_schedules(&self) -> Result<()> {
        ...
        for schedule in &active_schedules {
            ...
            let stream = match self.cameras_repo.get_stream_by_id(&schedule.stream_id).await? {
                Some(stream) => stream,
                None => {
                    warn!("Stream {} for schedule {} not found", 
            ...
            if self.recording_manager.is_recording_active(&schedule.id, &stream.id) {
                // Already recording, mark as should be recording
                should_be_recording.insert(
                    format!("{}-{}", schedule.id, stream.id), 
                    true
                );
                continue;
            }
            ...
            // Start new recording
            match self.recording_manager.start_recording(schedule, &stream).await {
                Ok(recording_id) => {
                    info!("Started recording {} for schedule {}", recording_id, schedule.id);
                ...
                Err(e) => {
                    error!("Failed to start recording for schedule {}: {}", schedule.id, e);
                }
            }
        }
        ...
        for schedule in &all_enabled_schedules {
            ...
            if self.recording_manager.is_recording_active(&schedule.id, &schedule.stream_id) {
                // Stop recording
                match self.recording_manager.stop_recording(&schedule.id, &schedule.stream_id).await {
                    Ok(_) => {
                        info!("Stopped recording for schedule {}", schedule.id);
                    },
                    Err(e) => {
                        error!("Failed to stop recording for schedule {}: {}", schedule.id, e);
                    }
                }
            }
        }
