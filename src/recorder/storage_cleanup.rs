...
use crate::db::models::recording_models::RecordingStats;
...
impl StorageCleanupService {
    ...
    pub async fn start(self: Arc<Self>) -> Result<()> {
        ...
        info!("Starting storage cleanup service with interval of {} seconds", 
    ...
    async fn cleanup_by_age(&self) -> Result<u64> {
        info!("Cleaning up recordings older than {} days", self.config.max_retention_days);

        // Calculate cutoff date
        let cutoff_date = Utc::now() - chrono::Duration::days(self.config.max_retention_days as i64);
        
        // Get recordings to delete
        let recordings = self.recordings_repo.get_recordings_to_prune(None, Some(cutoff_date)).await?;
    ...
    async fn cleanup_by_storage_usage(&self) -> Result<u64> {
        ...
        if disk_usage.percentage < self.config.max_disk_usage_percent as f64 {
            info!("Current disk usage is {}%, below threshold of {}%. No cleanup needed.",
                  disk_usage.percentage, self.config.max_disk_usage_percent);
            return Ok(0);
        }
        
        info!("Current disk usage is {}%, above threshold of {}%. Cleaning up oldest recordings.",
              disk_usage.percentage, self.config.max_disk_usage_percent);
        ...
        let target_usage = (disk_usage.total_bytes as f64 * 
                           (self.config.max_disk_usage_percent as f64 - 5.0) / 100.0) as u64;
        ...
        info!("Need to free approximately {} MB", bytes_to_free / 1024 / 1024);
        ...
        while deleted_bytes < bytes_to_free && processed < 1000 { // Safety limit
            // Get a batch of oldest recordings
            let recordings = self.recordings_repo
                .get_recordings_to_prune(None, None).await?;
        ...
        info!("Cleaned up {} recordings, freed {} MB", 
              delete_count, deleted_bytes / 1024 / 1024);
