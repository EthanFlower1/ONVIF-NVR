...
pub async fn create_webrtc_session(
    ...
    let ice_servers = vec![WebRTCIceServer {
...
pub async fn process_webrtc_offer(
    ...
    info!(
        "Processing WebRTC offer for session: {}",
        request.session_id
    );
    ...
    pipeline
        .add_many(&[&queue, &depay, &parse, appsink.upcast_ref()])
        .map_err(|e| {
    ...
    gst::Element::link_many(&[&queue, &depay, &parse, appsink.upcast_ref()]).map_err(|e| {
    ...
    tee_src_pad.link(&queue_sink_pad).map_err(|e| {
    ...
    let offer = match offer_sdp_type {
        RTCSdpType::Offer => RTCSessionDescription::offer(request.sdp).map_err(|e| {
    ...
    tokio::spawn(async move {
        while let Some(sample) = sample_receiver.recv().await {
            if let Err(err) = track_clone_for_receiver.write_sample(&sample).await {
                warn!(
                    "Failed to write sample to WebRTC track for session {}: {}",
                    session_id_for_receiver, err
                );
            }
        }
        info!(
            "Sample receiver task ended for session {}",
            session_id_for_receiver
        );
    });
        ...
        gst_app::AppSinkCallbacks::builder()
            .new_sample(move |appsink| {
                ...
                match tx.try_send(webrtc_sample) {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to send sample to channel: {} for session {}",
                            e,
                            session_id_for_debug.clone()
                        );
            ...
            .eos(move |_appsink| {
                info!(
                    "AppSink received EOS for session {}",
                    session_id_for_debug_2.clone()
                );
            })
            .build(),
    );
    ...
    let element_suffix = element_suffix_clone.clone();
    ...
    Box::pin(async move {
        match connection_state {
            ...
            webrtc::peer_connection::peer_connection_state::RTCPeerConnectionState::Closed => {
                info!("Cleaning up WebRTC connection for session: {}", session_id);
                ...
                if let Some(queue) = queue_opt {
                    if let Some(queue_sink_pad) = queue.static_pad("sink") {
                        // Unlink and release pad immediately
                    ...
                    if let Some(depay) = depay_opt { elements.push(depay); }
                    if let Some(parse) = parse_opt { elements.push(parse); }
                    if let Some(appsink) = appsink_opt { elements.push(appsink); }

                    // Send EOS and set elements to NULL state
                    ...
                    warn!("Queue element not found during cleanup for session {}", session_id);
                }

                // Remove peer connection from state
            ...
            _ => debug!("Connection state {:?} for session {}", connection_state, session_id),
        }
    })
}));
...
pub async fn add_ice_candidate(
    ...
    let peer_connection = {
        ...
        match peer_connections.get(&request.session_id) {
            ...
            None => {
                info!(
                    "ICE candidate received before peer connection setup for session {}",
                    request.session_id
                );
